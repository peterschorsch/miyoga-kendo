...
puts @admin_user.inspect
@admin_user = User.create_with(firstname: "Vince", lastname: "Chan", active: true, password_digest: User.digest("Vincechan3!"), role: "Admin", new_user: false, last_login: DateTime.now-1.week).find_or_create_by(email: "vincechan@yahoo.com")
puts @admin_user.inspect
@admin_user = User.create_with(firstname: "Miyoga", lastname: "Kendo Club", active: true, password_digest: User.digest("MiyogaKendo1!"), role: "Admin", new_user: false, last_login: DateTime.now-1.week).find_or_create_by(email: "miyoga.kendo.kai@gmail.com")
puts @admin_user.inspect
puts ""
...
puts @user.inspect
@user = User.create_with(firstname: "Tom", lastname: "Jones", active: true, password_digest: User.digest("Tomjones6!"), role: "User", new_user: false, last_login: DateTime.now-2.weeks).find_or_create_by(email: "tjones@msnglobal.net")
puts @user.inspect
puts ""
...
@social_media = SocialMedium.create_with(site_name: site_name, site_link: "https://mail.google.com/mail/u/0/#inbox", user_id: @admin_user.id,
...
@social_media = SocialMedium.create_with(site_name: site_name, site_link: "https://www.facebook.com/groups/672754536102008/", user_id: @admin_user.id,
    :image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )
...
@social_media = SocialMedium.create_with(site_name: site_name, site_link: "https://www.dropbox.com/home/Miyoga%20Kendo%20Kai", user_id: @admin_user.id,
    :image => ActionDispatch::Http::UploadedFile.new(:filename => File.basename(image_file),:tempfile => image_file,
    # detect the image's mime type with MIME if you can't provide it yourself.
    :type => MIME::Types.type_for(image_path).first.content_type
  )
...
puts @page.inspect
@page = Page.create_with(name: "News", path: "announcements_path", admin_path: "admin_announcements_path", index: 3, user_id: admin_user_id).find_or_create_by(name: "News")
puts @page.inspect
@page = Page.create_with(name: "Events", path: "events_path", admin_path: "admin_events_path", index: 4, user_id: admin_user_id).find_or_create_by(name: "Events")
puts @page.inspect
