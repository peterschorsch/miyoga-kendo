...
#puts "ADDRESS"
#@jcc_address = Address.create_with(address_line_1: "1016 W Belmont Ave", city: "Chicago", state_id: @jcc_state.id, zip_code: "60657", event_id: ).find_or_create_by(address_line_1: "1016 W Belmont Ave")
...
puts "CLASS SCHEDULE"
now = DateTime.now
@class_schedule = ClassSchedule.create_with(day_of_week: "Sunday", day_of_week_index: 1, start_time: Time.parse("06:30").in_time_zone('Central Time (US & Canada)'), end_time: Time.parse("07:45").in_time_zone('Central Time (US & Canada)'), cost_per_month: "30", content_id: @content.id).find_or_create_by(day_of_week: "Sunday")
puts @class_schedule.inspect
@class_schedule = ClassSchedule.create_with(day_of_week: "Friday", day_of_week_index: 6, start_time: Time.parse("20:30").in_time_zone('Central Time (US & Canada)'), end_time: Time.parse("21:45").in_time_zone('Central Time (US & Canada)'), cost_per_month: "30", content_id: @content.id).find_or_create_by(day_of_week: "Friday")
puts @class_schedule.inspect
puts ""
